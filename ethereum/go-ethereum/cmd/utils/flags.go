...
var (
	...
	////////////////////////////////////////
	/// each node has a different identity for pbft consensus algorithm. --Zhiguo
	// 	PBFTIdFlag = DirectoryFlag{
	// 		Name:  "pbftid",
	// 		Usage: "Identity number of this node for the PBFT consensus algorithm (integer from 0 to N-1)",
	// 		/// no default value for pbftid.
	// ///		Value: DirectoryString{node.DefaultDataDir()},
	// 	}
	////////////////////////////////////////
	//=> refer NetWorkIdFlag, change PBFTIdFlag --Agzs
	PBFTIdFlag = cli.Uint64Flag{
		Name:  "pbftid",
		Usage: "Identity number of this node for the PBFT consensus algorithm (integer from 0 to N-1)",
		Value: eth.DefaultConfig.PbftId,
		/// no default value for pbftid.
		///		Value: DirectoryString{node.DefaultDataDir()},
	}
	//=> end. --Agzs
	...
	//=> refer NetWorkIdFlag, change BlockchainIdFlag --Agzs 12.25
	BlockchainIdFlag = cli.Uint64Flag{
		Name:  "blockchainid",
		Usage: "Blockchain identifier (integer, path from root to current blockchain)",
		Value: eth.DefaultConfig.BlockchainId,
	}
	//=> end. --Agzs
	...
	NodeIndexFlag = cli.UintFlag{
		Name:  "nodeindex",
		Usage: "Index of local node in a blockchain",
	}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	if ctx.GlobalIsSet(NodeIndexFlag.Name) {
		cfg.NodeIndex = uint32(ctx.GlobalInt(NodeIndexFlag.Name))
		node.NodeIndex = cfg.NodeIndex
		fmt.Println("nodeindex", node.NodeIndex)
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	////////////////////////////////////////
	//=> get blockchainID --Agzs 12.25
	if ctx.GlobalIsSet(BlockchainIdFlag.Name) {
		cfg.BlockchainId = ctx.GlobalUint64(BlockchainIdFlag.Name)
	}
	///	Zhiguo
	if ctx.GlobalIsSet(PBFTIdFlag.Name) {
		cfg.PbftId = ctx.GlobalUint64(PBFTIdFlag.Name)
	}
	////////////////////////////////////////
