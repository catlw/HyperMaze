...
////xiaobei 2.4
var StopSend = make(chan struct{}, 1)
...
type Balances struct {
	...
	PlainBalance *big.Int       `json:"plainbalance"`
	ZKBalance    common.Hash    `json:"zkbalance"`
}

func (s *PublicTransactionPoolAPI) GetBalances(ctx context.Context, args SendTxArgs3) (Balances, error) {
	...
	zk_balance := state.GetCMTBalance(Address)
	balances := Balances{
		Address,
		balance,
		zk_balance,
...
func (s *PublicTransactionPoolAPI) SendConvertTransaction(ctx context.Context, args SendTxArgs3) (common.Hash, error) {
	...
	fmt.Println("convert para")
	fmt.Println(SN.CMT, SN.SN, SN.Random, SN.Value, newCMT, newSN, newRandom, newValue)

	//genProofStart := time.Now()
	zkProof := zktx.GenConvertProof(SN.CMT, SN.SN, SN.Random, SN.Value, newCMT, newSN, newRandom, newValue)
	...
	if err == nil {
		...
		zktx.Stage = zktx.TxConvert
		SNS := zktx.SequenceS{*zktx.SequenceNumber, *zktx.SequenceNumberAfter, zktx.SNFD, nil, nil, zktx.TxConvert}
...
func (s *PublicTransactionPoolAPI) SendRedeemTransaction(ctx context.Context, args SendTxArgs3) (common.Hash, error) {
	...
	if SN.Value < args.Value.ToInt().Uint64() {
		s := fmt.Sprintln("insufficient zkbalance for redeem has %d, expect %d", SN.Value, args.Value.ToInt().Uint64())
		return common.Hash{}, errors.New(s)
	}
	newValue := SN.Value - args.Value.ToInt().Uint64()
	...
	fmt.Println(SN.CMT, SN.SN, SN.Random, SN.Value, newCMT, newSN, newRandom, newValue)
	//genProofStart := time.Now()
	zkProof := zktx.GenRedeemProof(SN.CMT, SN.SN, SN.Random, SN.Value, newCMT, newSN, newRandom, newValue)
	...
	if err == nil {
		zktx.SequenceNumber = zktx.SequenceNumberAfter
		zktx.SequenceNumberAfter = &zktx.Sequence{SN: newSN, CMT: newCMT, Random: newRandom, Value: newValue}
		zktx.Stage = zktx.TxRedeem
		SNS := zktx.SequenceS{*zktx.SequenceNumber, *zktx.SequenceNumberAfter, zktx.SNFD, nil, nil, zktx.TxRedeem}
		SNSBytes, err := rlp.EncodeToBytes(SNS)
...
func (s *PublicTransactionPoolAPI) SendDepositTransaction(ctx context.Context, args SendTxArgs3) (common.Hash, error) {
	...
	SNFD := zktx.NewRandomHash()
	RFD := zktx.NewRandomHash()

	CMTFD := zktx.GenCMTFD(args.Value.ToInt().Uint64(), toAddress.Bytes(), SNFD.Bytes(), RFD.Bytes(), SN.SN.Bytes()) //tbd
	tx.SetZKCMTfd(*CMTFD)
	fmt.Println("deposit para")
	...
	zkProof := zktx.GenDepositProof(SN.CMT, SN.Value, SN.Random, args.Value.ToInt().Uint64(), toAddress, SNFD, RFD, SN.SN, CMTFD, newValue, newSN, newRandom, newCMT)
	//genProofStart := time.Now()
...
func (s *PrivateAccountAPI) SendTransaction(ctx context.Context, args SendTxArgs, passwd string) (common.Hash, error) {
	fmt.Printf("-------SendTransaction") ////xiaobei 1.10
	// Look up the wallet containing the requested signer
	account := accounts.Account{Address: args.From}
...
func (s *PrivateAccountAPI) SignAndSendTransaction(ctx context.Context, args SendTxArgs, passwd string) (common.Hash, error) {
	fmt.Printf("-------SignAndSendTransaction") ////xiaobei 1.10
...
func (s *PublicTransactionPoolAPI) GetTxProofByHash(ctx context.Context, hash common.Hash) ([]hexutil.Bytes, error) {
	...
	if err != nil {
		fmt.Println("EncodeToBytes error", err)
		return nil, nil
	}

	//fmt.Println("b bytes", blockBytes)
	//txHash := block.TxHash()

	var tx *types.Transaction
	if tx, _, err = getTransaction(s.b.ChainDb(), s.b, hash); err != nil {
		fmt.Println("Failed to retrieve transaction", "hash", hash, "err", err)
		return nil, nil
	} else if tx == nil {
		fmt.Println("tx does not exist")
		return nil, nil
	}

	txBytes, err := rlp.EncodeToBytes(tx)

	txProof := make([]hexutil.Bytes, 2)
	txProof[0] = txBytes
	txProof[1] = blockBytes
...
func (s *PublicTransactionPoolAPI) GetTxProofByProof(ctx context.Context, proof []hexutil.Bytes) ([]hexutil.Bytes, error) {
	...
	block1 := new(types.Block)
	stream := rlp.NewStream(bytes.NewReader(last[:]), 0)
	err = stream.Decode(block1)
	if err != nil {
		fmt.Println("decode block err", err)
		return nil, err
	...
	newproof := append(proof, blockBytes)

	numberb, _ := rlp.EncodeToBytes(block.Number().Uint64())
	newproof = append(newproof, numberb)
	//newproof = append(newproof, rlp.EncodeToBytes(block.Number().Uint64()))
	return newproof, nil
}
...
func (args *SendTxArgs2) setDefaults(ctx context.Context, b Backend) error {
	if args.Gas == nil {
...
func (args *SendTxArgs2) toTransaction() *types.Transaction {
	...
	if args.ToID == "" && args.ToIndex == 0 {
		to = &DHibeAddress{node.ID, node.NodeIndex}
	} else if args.ToID != "" && args.ToIndex != 0 {
		to = &DHibeAddress{args.ToID, args.ToIndex}
	}
	to = &DHibeAddress{args.ToID, args.ToIndex}
	toAddress := to.Address()
	//fmt.Println(toAddress)
	//tx = types.NewTransaction(uint64(*args.Nonce), toAddress, (*big.Int)(args.Value), (*big.Int)(args.Gas), (*big.Int)(args.GasPrice), args.Data)

	tx = types.NewTransaction(uint64(*args.Nonce), toAddress, (*big.Int)(args.Value), (*big.Int)(args.Gas), big.NewInt(0), args.Data)
	if args.FromID == "" && args.FromIndex == 0 {
		from = &DHibeAddress{node.ID, node.NodeIndex}
	} else if args.FromID != "" && args.FromIndex != 0 {
...
func submitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	//fmt.Printf("--------submitTransaction") ////xiaobei 1.10
	if err := b.SendTx(ctx, tx); err != nil {
		fmt.Printf("----send tx meet err,tx : %x", tx.Hash().Hex())
		fmt.Printf("----send tx meet err,err is ", err)
		return common.Hash{}, err
	}
	if tx.To() == nil {
...
func submitZKTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	b.SendBatch([]*types.Transaction{tx})
	log.Info("Submitted transaction", "fullhash", tx.Hash().Hex(), "recipient", tx.To().Hex())
	return tx.Hash(), nil
}

////xiaobei 1.30
func (s *PublicTransactionPoolAPI) SendBatchTransactions(ctx context.Context, args SendTxArgs) {
	log.Info("-------send batch transaction is called!")
	go s.StartSend(ctx, args)
}

func (s *PublicTransactionPoolAPI) StartSend(ctx context.Context, args SendTxArgs) {
	var err error
	for {
		_, err = s.SendTransaction(ctx, args)
		time.Sleep(2 * time.Second)
		if err != nil {
			fmt.Printf("------send transaction meet err!", err)
			return
		}
		select {
		case <-StopSend:
			return
		default:
			log.Info("Continue send transaction!")
		}
	}
}

func (s *PublicTransactionPoolAPI) StopSend() {
	log.Info("Stop send transaction!")
	StopSend <- struct{}{}
}

type DHibeAddress struct {
	ID    string
	Index uint32
}

func (address *DHibeAddress) Address() common.Address {
	if len(address.ID) > 16 {
		fmt.Println("Too long ID")
		return common.Address{}
	}
	bytes := make([]byte, 20)

	for i := range address.ID {
		bytes[i] = address.ID[i]
	}

	intbytes := make([]byte, 4)
	binary.BigEndian.PutUint32(intbytes, address.Index)

	for i := 0; i < 4; i++ {
		bytes[16+i] = intbytes[i]
	}
...
func (s *PublicTransactionPoolAPI) RemoveTx() bool {
...
func (s *PublicTransactionPoolAPI) KeyCount(ctx context.Context) (uint32, error) {
	return node.KeyCount, nil
...
func (s *PublicTransactionPoolAPI) TestSendTransaction2(ctx context.Context, args SendTxArgs2) (uint32, error) {
	var i uint64
	value := (*big.Int)(args.Value)
	times := args.TxInterval * 1000 / args.TxPeriod
	var count uint32 = 0
	for i < 1<<62 {
		_, err := s.SendTransaction2(ctx, args)
		if err != nil {
			break
		}
		...
		<-time.After(time.Duration(args.TxPeriod) * time.Millisecond)
		count = count + 1
		if count >= times {
			break
		}
...
func (s *PublicTransactionPoolAPI) SendTransaction2(ctx context.Context, args SendTxArgs2) (common.Hash, error) {

	if hibe.DHibePrivatekey() == nil || hibe.DHibePubkey() == nil || hibe.Random == nil {
	...
	tx.SetLevel(node.LocalLevel)
	fmt.Println("id", args.ToID, node.ID)
	fmt.Println("nodeindex", args.ToIndex)
	if PID(args.ToID) == PID(node.ID) {
		tx.SetTxType(types.TxDhibe)
	} else {
		tx.SetTxType(types.TxCrossChain)
...
func (s *PublicTransactionPoolAPI) RequestCrossChainTransation(ctx context.Context, args SendTxArgs2) (common.Hash, error) {

	if hibe.DHibePrivatekey() == nil || hibe.DHibePubkey() == nil || hibe.Random == nil || args.Proof == nil {
		fmt.Println("private key or proof does not exist")
		return common.Hash{}, nil
	}
	...
	tx.SetRequestCrossAddress()

	types.DHibeSignTx(tx)

	tx.SetProof(args.Proof)
	err := core.ValidateCrossChainTx(tx, nil)
	if err != nil {
		return common.Hash{}, err
	}
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	fmt.Printf("------add tx2 %+v\n", signed)
	return submitTransaction(ctx, s.b, signed)
	// if f == 0 {
	// 	fmt.Printf("-------(s *PublicTransactionPoolAPI) SendTransaction") ////xiaobei 1.10
	// 	// Look up the wallet containing the requested signer
	// 	account := accounts.Account{Address: args.From}
	...
	// 	return submitTransaction(ctx, s.b, signed)
	// } else {
	// 	return s.SendBatchTransactions(ctx, args, 0)
	// }
...
func (s *PublicTransactionPoolAPI) SendRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (string, error) {
	fmt.Printf("----sendRawTransaction") ////xiaobei 1.10
	tx := new(types.Transaction)
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, gasPrice, gasLimit *hexutil.Big) (common.Hash, error) {
	fmt.Printf("------resend") ////xiaobei 1.10
