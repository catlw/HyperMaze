...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	...
	var (
		...
		events2       = make([]interface{}, 0, len(chain))
	...
	for i, block := range chain {
		...
		case CanonStatTy:
			...
			events2 = append(events2, ChainEvent{block, block.Hash(), logs})
		...
		case SideStatTy:
			...
			blockInsertTimer.UpdateSince(bstart)
			events2 = append(events2, ChainSideEvent{block})
		}
		stats.processed++
	...
	if consensus.PBFTEngineFlag { //=> --Agzs 18.03.28
		////xiaobei --1.17
		go func() {
			if fetcher.FetcherFlag {
				log.Info("------fetcher.FetcherFlag is true")
				events.ManagerImpl.Queue() <- CommittedEvent2{}
				fetcher.FetcherFlag = false
				time.Sleep(time.Millisecond)
				if !ViewChangeFlag {
					bc.postChainEvents(events2, coalescedLogs)
				}
			} else {
				log.Info("------fetcher.FetcherFlag is false")
				if !ViewChangeFlag {
					bc.postChainEvents(events2, coalescedLogs)
				}
			}
		...
		go bc.postChainEvents(events2, coalescedLogs)
	}
...
func (st *insertStats) report(chain []*types.Block, index int) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		if consensus.PBFTEngineFlag { //=> --Agzs 18.03.28
			ImportNum = ImportNum + st.processed ////xiaobei 1.2
		}
		if st.queued > 0 {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	//		bc.eventMux.Post(core.RootChainBlock{Block: block})
	//	}
	return nil
}
...
func (bc *BlockChain) postChainEvents(events2 []interface{}, logs []*types.Log) {
...
//=>TODO. --Agzs
func (bc *BlockChain) GetBlockchainInfo() (*types.BlockchainInfo, error) {
	block := bc.currentBlock
	if block == nil {
		return &types.BlockchainInfo{Height: 0}, nil
	}
	blockchainSize := block.Header().Number.Uint64() + 1

	lastBlock := bc.CurrentBlock()
	//log.Info("-------bc.CurrentBlock() is %+v", lastBlock)
	info := bc.getBlockchainInfoForBlock(blockchainSize, lastBlock)
	return info, nil
}

//=>TODO. --Agzs
func (bc *BlockChain) getBlockchainInfoForBlock(height uint64, block *types.Block) *types.BlockchainInfo {
	hash := block.Hash().Bytes()
	prevHash := bc.GetBlockByNumber(height - 2).Hash().Bytes()
