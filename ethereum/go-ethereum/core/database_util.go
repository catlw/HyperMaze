...
var (
	...
	headerTxMetaSuffix       = []byte{0x02}
	AddressNonceSuffix       = []byte{0x03}
	RootChainBlockHashSuffix = []byte{0x04}
	TxProofSuffix            = []byte{0x05}
...
func WriteTransactions(db ethdb.Database, block *types.Block) error {
	...
	for i, tx := range block.Transactions() {
		...
		if tx.TxType() == types.TxHeader {
			for _, headerHash := range tx.Headers() {
				headerTxmeta := struct {
					BlockHash     common.Hash
					BlockIndex    uint64
					...
					BlockHash:     block.Hash(),
					BlockIndex:    block.NumberU64(),
					HeaderTxHash:  tx.Hash(),
					HeaderTxIndex: uint64(i),
				}
				headerTxmetadata, err := rlp.EncodeToBytes(headerTxmeta)
				if err != nil {
					return err
				}
