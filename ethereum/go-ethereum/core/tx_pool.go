...
func ValidateCrossChainTx(tx *types.Transaction, db ethdb.Database) error {
	...
	depth := node.TotalLevel
	proof := tx.Proof()
	if len(proof) != int(depth)+1 {
		fmt.Println("wrong proof size")
		return errors.New("wrong proof size")
	}
	oldtx := new(types.Transaction)
	stream2 := rlp.NewStream(bytes.NewReader(proof[0][:]), 0)
	err = stream2.Decode(oldtx)
	if err != nil {
		fmt.Println("decode tx err", err)
		return err
	}
	...
	if VerifyProof(tx.Proof(), db) == false {
	...
	if tx.Recipient() != oldtx.CrossChainRecipient() {
		fmt.Println("receipent are different")
		return errors.New("wrong sender")
	}

	if tx.Value().Cmp(oldtx.Value()) > 0 {
...
/*
func proofUsed(proof []byte) bool {
	proofhash := types.Hash(proof)
}
*/
...
func VerifyProof(proof [][]byte, db ethdb.Database) bool {
	...
	if containtxflag == false {
		fmt.Println("block does not contain tx")
		return false
	}
	...
	if ValidateProofAlreadyExist(proof[0], db) == true {
		fmt.Println("proof was used before")
		return false
	}
...
func ValidateProofRoot(rootproof []byte, db ethdb.Database) bool {
	if db == nil {
		return true
	}
	if _, err := db.Get(append(rootproof, RootChainBlockHashSuffix...)); err != nil {
		return false
	}
	return true
}

func ValidateProofAlreadyExist(proof []byte, db ethdb.Database) bool {
	if db == nil {
		return false
	}
	proofhash := types.Hash(proof)
	if _, err := db.Get(append(proofhash.Bytes(), TxProofSuffix...)); err != nil {
		return false
	}
	return true
}

func checkHeaderExist(tx *types.Transaction, hash common.Hash) bool {
	headers := tx.Headers()
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	pool.gasPrice = big.NewInt(0)               //TBD
	...
	if tx.TxType() == types.TxCrossChain && PID(recepient.ID()) == node.ID {
		err := ValidateCrossChainTx(tx, DB)
		if err != nil {
			return err
		}
	}
	if tx.TxType() == types.TxCrossChain && len(tx.Proof()) != 0 {
		proof := tx.Proof()
		proofhash := types.Hash(proof[0])
		DB.Put(append(proofhash.Bytes(), TxProofSuffix...), proof[0])
	}
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	if uint64(len(pool.all)) >= pool.config.GlobalSlots+pool.config.GlobalQueue {
		fmt.Printf("----transaction pool is full\n")
		// If the new transaction is underpriced, don't accept it
		if pool.priced.Underpriced(tx, pool.locals) {
			fmt.Printf("\nDiscarding underpriced transaction:%x", hash) ////xiaobei 2.5
	...
	if err != nil {
		fmt.Printf("------replace, err := pool.enqueueTx(hash, tx)\n", err) ////xiaobei 2.5
		return false, err
	}
	if local {
...
func (pool *TxPool) enqueueTx(hash common.Hash, tx *types.Transaction) (bool, error) {
	...
	if old != nil {
		delete(pool.all, old.Hash())
		fmt.Printf("---Discard any previous transaction and mark this")
		pool.priced.Removed()
...
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) {
	...
	if !inserted {
		...
		pool.priced.Removed()
		fmt.Printf("-----An older transaction was better, discard this\n")
		pendingDiscardCounter.Inc(1)
		return
	}
	...
	if old != nil {
		delete(pool.all, old.Hash())
		pool.priced.Removed()
		fmt.Printf("-----promoteTx delete the old one\n")
		pendingReplaceCounter.Inc(1)
	}
	// Failsafe to work around direct pending inserts (tests)
	if pool.all[hash] == nil {
		fmt.Printf("-----pool.all[hash] == nil ,put the tx into the pool\n")
		pool.all[hash] = tx
		pool.priced.Put(tx)
	}
	...
	//fmt.Printf("-----promoteTx") ////xiaobei 1.10
...
func (pool *TxPool) AddLocal(tx *types.Transaction) error {
	//fmt.Printf("---------AddLocal") ////xiaobei 1.10
	err := pool.addTx(tx, !pool.config.NoLocals)
	if err != nil {
		fmt.Printf("-----addLocal meet err", err)
	}
	return err
}
...
func (pool *TxPool) AddRemote(tx *types.Transaction) error {
	fmt.Printf("---------AddRemote") ////xiaobei 1.10
...
func (pool *TxPool) addTx(tx *types.Transaction, local bool) error {
	...
	if err != nil {
		fmt.Printf("\n----replace, err := pool.add(tx, local),err=", err)
		return err
	}
...
func (pool *TxPool) Remove(hash common.Hash) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	fmt.Printf("-------removes the transaction with the given hash from the pool.\n")
	pool.removeTx(hash)
}
...
func (pool *TxPool) RemoveBatch(txs types.Transactions) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	fmt.Printf("-------removes all given transactions from the pool.\n")
	for _, tx := range txs {
		pool.removeTx(tx.Hash())
	}
}
...
func (pool *TxPool) removeTx(hash common.Hash) {
	...
	delete(pool.all, hash)
	pool.priced.Removed()
	fmt.Printf("------remove the tx from all %x\n", hash)
	...
	for _, addr := range accounts {
		...
		/*
			for _, tx := range list.Forward(state.GetNonce(addr)) {
				...
				fmt.Printf("------Removed old queued transaction:%x\n", hash)
				delete(pool.all, hash)
				pool.priced.Removed()
			}
		*/
		...
		for _, tx := range drops {
			...
			fmt.Printf("Removed unpayable queued transaction:%x\n", hash)
			delete(pool.all, hash)
			pool.priced.Removed()
		...
		if !pool.locals.contains(addr) {
			for _, tx := range list.Cap(int(pool.config.AccountQueue)) {
				hash := tx.Hash()
				delete(pool.all, hash)
				fmt.Printf("------Removed cap-exceeding queued transaction\n")
				pool.priced.Removed()
				...
				for pending > pool.config.GlobalSlots && pool.pending[offenders[len(offenders)-2]].Len() > threshold {
					for i := 0; i < len(offenders)-1; i++ {
						...
						for _, tx := range list.Cap(list.Len() - 1) {
							...
							hash := tx.Hash()
							delete(pool.all, hash)
							fmt.Printf("-----Drop the transaction from the global pools too")
							pool.priced.Removed()
			...
			for pending > pool.config.GlobalSlots && uint64(pool.pending[offenders[len(offenders)-1]].Len()) > pool.config.AccountSlots {
				for _, addr := range offenders {
					...
					for _, tx := range list.Cap(list.Len() - 1) {
						...
						delete(pool.all, hash)
						pool.priced.Removed()
						fmt.Printf("-------Removed fairness-exceeding pending transaction\n")
		...
		for drop := queued - pool.config.GlobalQueue; drop > 0 && len(addresses) > 0; {
			...
			if size := uint64(list.Len()); size <= drop {
				for _, tx := range list.Flatten() {
					fmt.Printf("----Drop all transactions if they are less than the overflow\n")
					pool.removeTx(tx.Hash())
				}
				drop -= size
			...
			for i := len(txs) - 1; i >= 0 && drop > 0; i-- {
				fmt.Printf("----drop only last few transactions\n")
...
func (pool *TxPool) demoteUnexecutables(state *state.StateDB) {
	...
	for addr, list := range pool.pending {
		...
		for _, tx := range list.Forward(nonce) {
			...
			delete(pool.all, hash)
			fmt.Printf("-------Removed old pending transaction,tx is %x\n", hash)
			pool.priced.Removed()
		}
		...
		for _, tx := range drops {
			...
			delete(pool.all, hash)
			pool.priced.Removed()
			fmt.Printf("--------Removed unpayable pending transaction")
			...
			for addr := range pool.queue {
				...
				if time.Since(pool.beats[addr]) > pool.config.Lifetime {
					for _, tx := range pool.queue[addr].Flatten() {
						fmt.Printf("----Any non-locals old enough should be removed\n")
						pool.removeTx(tx.Hash())
					}
				}
			}
