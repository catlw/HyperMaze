...
func Sender(signer Signer, tx *Transaction) (common.Address, error) {
	...
	sender, err := signer.PublicKey(tx)
	if err != nil {
...
type DHibeSigner struct {
	...
	publicKey  *hibe.MasterPublicKey
}

func NewDHibeSigner() DHibeSigner {
	if hibe.PrivateKey == nil || hibe.MasterPubKey == nil {
		fmt.Println("hibe privatekey does not exist")
		return DHibeSigner{}
	}
	return DHibeSigner{hibe.PrivateKey, hibe.MasterPubKey}
}

func (s DHibeSigner) Equal(s2 Signer) bool {
	_, ok := s2.(DHibeSigner)
	return ok
}

func (s DHibeSigner) PublicKey(tx *Transaction) ([]byte, error) {
	signature := tx.data.HibeSig
	ID := tx.ID()
...
func (s DHibeSigner) WithSignature(tx *Transaction, sig []byte) (*Transaction, error) {

	cpy := &Transaction{data: tx.data}
	signature := make([]byte, len(sig))
	copy(signature[:], sig[:])
	cpy.data.HibeSig = &hibe.CompressedSIGBytes{signature}
	return cpy, nil
}

func (s DHibeSigner) Hash(tx *Transaction) common.Hash {
...
func HibeHash(tx *Transaction) common.Hash {
	switch tx.TxType() {
	case TxDhibe:
		return rlpHash([]interface{}{
			tx.data.AccountNonce,
	...
	case TxCrossChain:
		return rlpHash([]interface{}{
			...
			tx.data.Level,
			tx.data.Sender,
			tx.data.CrossChainSender,
			tx.data.CrossChainRecipient,
		})
	default:
		return common.Hash{}
