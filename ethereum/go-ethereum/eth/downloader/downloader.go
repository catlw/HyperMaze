...
func (d *Downloader) RegisterPeer(id string, version int, peer Peer) error {
	...
	//log.Info("------RegisterPeer") ////xiaobei 1.9
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	//log.Info("----syncWithPeer") ////xiaobei --1.3
	...
	defer func() {
		...
		if err != nil {
			...
			if consensus.PBFTEngineFlag { //=> --Agzs 18.03.28
				////xiaobei --1.3
				if !pbft.StateTransferFlag {
					str := strconv.Itoa(core.ImportNum)
					importnum, err := strconv.ParseUint(str, 10, 64)
					if err != nil {
						panic(err)
					}
					pbft.N = pbft.N + importnum
					fmt.Println("-----lastExec = ", pbft.N)
				} else {
					pbft.StateTransferFlag = false
				}
				////
			}
		}
...
func (d *Downloader) DeliverHeaders(id string, headers []*types.Header) (err error) {
	...
	//=> if there reports error, you can delete gyeongchingtarn//ethereum/go-ethereum/vendor file   --Agzs
}
