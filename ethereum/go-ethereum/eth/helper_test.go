...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, error) {
	...
	pm, err := NewProtocolManager(gspec.Config, mode, DefaultConfig.BlockchainId, DefaultConfig.NetworkId, 1000, evmux, &testTxPool{added: newtx}, engine, blockchain, db)
	if err != nil {
		return nil, err
	}
	pm.Start()
...
func (p *testPeer) handshake(t *testing.T, td *big.Int, head common.Hash, genesis common.Hash) {
	msg := &statusData{
		...
		BlockchainId:    DefaultConfig.BlockchainId,
		TD:              td,
		CurrentBlock:    head,
