...
public:
    ...
    std::shared_ptr<digest_variable<FieldT>> tx_root; // 256位的随机数serial number    
    std::shared_ptr<digest_variable<FieldT>> state_root; // 256位的随机数r    
    ...
    void generate_r1cs_witness(const NoteS& note_s, const Note& note_old, const Note& note, uint256 tx_root_data, uint256 state_root_data) { // 为变量生成约束
        ...
        tx_root->bits.fill_with_bits(this->pb, uint256_to_bool_vector(tx_root_data));
        state_root->bits.fill_with_bits(this->pb, uint256_to_bool_vector(state_root_data));
    }
};